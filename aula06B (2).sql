...
-- Aqui usamos SELECT por coluna, então listamos tudo EXCETO o que foi proibido

GRANT SELECT (ID, name, dept_name)
ON INSTRUCTOR
TO User_B;
-- não incluímos "salary", então ele não terá acesso a esse atributo

GRANT SELECT (ID, course_id, sec_id, semester, year)
ON TAKES
TO User_B;
-- não incluímos "grade", então ele não poderá visualizar a nota
...
ON SECTION
TO User_C;
-- aqui limitamos tanto a consulta quanto a modificação
...
-- então criamos uma VIEW com filtro
